...
@section('content')       
	...
	<?php
		...
		{
			if($a->file != ""){
				echo '<h3>'.$a->title.'</h3><img src="../../upload/'.$a->file.'"><small>'.$a->content.'<br/>'.$a->price.'<br/>'.$a->region.'<br/>'.$a->id_user.'</small>';
				?><div id='edit'><?php
				if(Auth::user()->id == $a->id_user){
					?><a onClick="show('update')">Modifier</a><br/><?php
					?><a href="../deletesale/<?php echo $id ?>" onclick="return(confirm('Etes-vous sûr de vouloir supprimer cette entrée?'));">Supprimer</a><?php
				}
				?></div><?php
			}else{
				echo '<h3>'.$a->title.'</h3><img src="../../upload/notimg.png"><small>'.$a->content.'<br/>'.$a->price.'<br/>'.$a->region.'<br/>'.$a->id_user.'</small>'
				?><div id='edit'><?php
				if(Auth::user()->id == $a->id_user){
					?><a onClick="show('update')">Modifier</a><br/><?php
					?><a href="../deletesale/<?php echo $id ?>" onclick="return(confirm('Etes-vous sûr de vouloir supprimer cette entrée?'));">Supprimer</a><?php
	...
	<div id="update">
		{{Form::open(array('url' => '/editsale/'.$id,'files'=>true))}}
