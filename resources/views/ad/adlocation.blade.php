...
@section('content')       
	<div id="ad">
	<?php
		$article = DB::table('location')->where('id', $id)->get();
		foreach ($article as $a)
		{
			if($a->file != ""){
				echo '<h3>'.$a->title.'</h3><img src="../../upload/'.$a->file.'"><small>'.$a->content.'<br/>'.$a->region.'<br/>'.$a->price.$a->time.'<br/>'.$a->id_user.'</small>';
				?><div id='edit'><?php
				...
				echo '<h3>'.$a->title.'</h3><img src="../../upload/notimg.png"><small>'.$a->content.'<br/>'.$a->region.'<br/>'.$a->price.$a->time.'<br/>'.$a->id_user.'</small>'
				?><div id='edit'><?php
				if(Auth::user()->id == $a->id_user){
					?><a onClick="show('update')">Modifier</a><br/><?php
					?><a href="../deletelocation/<?php echo $id ?>" onclick="return(confirm('Etes-vous sûr de vouloir supprimer cette entrée?'));">Supprimer</a><?php
	...
	<div id="update">
		{{Form::open(array('url' => '/editlocation/'.$id,'files'=>true))}}
