...
@section('content')       
	...
	<?php
		...
		{
			if($a->file != ""){
				echo '<h3>'.$a->title.'</h3><img src="../../upload/'.$a->file.'"><small>'.$a->content.'<br/>'.$a->region.'<br/>'.$a->id_user.'</small>';
				?><div id='edit'><?php
				...
				echo '<h3>'.$a->title.'</h3><img src="../../upload/notimg.png"><small>'.$a->content.'<br/>'.$a->region.'<br/>'.$a->id_user.'</small>'
				?><div id='edit'><?php
				if(Auth::user()->id == $a->id_user){
					?><a onClick="show('update')">Modifier</a><br/><?php
					echo '<a href="../deletegift/'.$id.'">Supprimer</a>';
	...
	<div id="update">
		{{Form::open(array('url' => '/editgift/'.$id,'files'=>true))}}
